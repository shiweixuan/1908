...
<body>
	<center><h2>管理员添加页面</h2></center><hr/>
<form action="{{url('/admin/store')}}" method="post" class="form-horizontal" role="form">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" id="firstname" 
				   placeholder="请输入管理员名称" name="a_name">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" id="firstname" 
				   placeholder="请输入邮箱" name="a_email">
...
<script>
	$('input[type="button"]').click(function(){
		...
		if(!reg.test(a_name)){
			$('input[name="a_name"]').next().html('管理员名称由文字母数字组成且不能为空');
			return;
		}
	...
	$("input[name='a_name']").blur(function(){
		...
		var reg=/^[\u4e00-\u9fa50-9A-Za-z_]+$/;
		if(!reg.test(a_name)){
			$(this).next().html('管理员名称由文字母数字组成且不能为空');
			return;
		}
